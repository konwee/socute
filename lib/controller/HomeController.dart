...
class HomeController {
  ...
  static updateCountItem(
      {required BuildContext context, required itemModel data}) {
    if (inputItem.any((element) => element.idItem == data.idItem)) {
      var index =
          inputItem.indexWhere((element) => element.idItem == data.idItem);
  ...
  static getProductAll() async {
    var headers = {
      'Authorization':
          'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ0eXBlIjoxLCJzdWIiOiJzYXJhd3V0LmJjczU5QHVicnUuYWMudGgiLCJkYXRlX3RpbWUiOiIyMDIzLTAyLTA5IDIyOjA2OjE5IiwiZXhwIjoxNjc1OTU4Nzc5fQ.7ZWu_jaT1Pc3X8uOJpSVmKb_r-N1oPzqhyyR-NusU2o'
    };

    var request =
        http.Request('GET', Uri.parse('http://10.30.2.32:5000/GetProductAll'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var data = await response.stream.bytesToString();
      var responsejson = json.decode((data));
      getProductAllList = await (responsejson["data"] as List)
          .map((e) => GetProductAllList.fromMap(e))
          .toList();
